...
// i18n dictionaries inline for client component
const blogPageDict: Record<string, Record<string, string>> = {
  pt: {
    badge: 'BLOG',
    heroTitle: 'Insights do',
    heroAccent: 'Infinity Blog',
    heroSubtitle: 'Artigos, guias e novidades sobre tecnologia, e-commerce e inovação.',
    ...
    newsletterTitle: 'Receba nossos',
    newsletterAccent: 'Insights',
    newsletterDesc: 'Assine nossa newsletter e receba os melhores artigos sobre tecnologia e inovação.',
    newsletterSuccess: '✅ Inscrito com sucesso!',
    newsletterBtn: 'Assinar →',
    newsletterPlaceholder: 'seu@email.com',
  },
  es: {
    badge: 'BLOG',
    heroTitle: 'Insights del',
    heroAccent: 'Infinity Blog',
    heroSubtitle: 'Artículos, guías y novedades sobre tecnología, e-commerce e innovación.',
    searchPlaceholder: '🔍 Buscar artículos...',
    all: 'Todos',
    loading: 'Cargando artículos...',
    noArticles: 'Ningún artículo encontrado',
    tryAnother: 'Intenta con otra búsqueda o categoría.',
    readArticle: 'Leer artículo',
    readTime: 'min de lectura',
    newsletterTitle: 'Recibe nuestros',
    newsletterAccent: 'Insights',
    newsletterDesc: 'Suscríbete a nuestro newsletter y recibe los mejores artículos sobre tecnología e innovación.',
    newsletterSuccess: '✅ ¡Suscrito con éxito!',
...
export default function BlogPage() {
  ...
  const t = blogPageDict[locale] || blogPageDict.pt;
  ...
  const formatDate = (d: string) => new Date(d).toLocaleDateString(dateLocale, { day: '2-digit', month: 'short', year: 'numeric' });
  ...
  return (
    <main style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--bg-primary)' }}>
      ...
      <section style={{ padding: 'clamp(80px, 10vw, 120px) clamp(16px, 4vw, 48px) 60px', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', padding: '6px 16px', backgroundColor: 'var(--bg-secondary)', color: 'var(--accent-primary)', borderRadius: '20px', fontSize: '13px', fontWeight: 600, marginBottom: '24px', border: '1px solid var(--bg-tertiary)', letterSpacing: '1px', textTransform: 'uppercase' }}>{t.badge}</div>
              ...
              >
                ...
                <div style={{ padding: '28px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div className="text-secondary" style={{ fontSize: '13px', marginBottom: '12px' }}>
                    {formatDate(post.published_at)} • {post.reading_time || 5} {t.readTime}
                  </div>
      ...
      <section style={{ padding: '80px clamp(16px, 4vw, 48px)', backgroundColor: 'var(--bg-secondary)', borderTop: '1px solid var(--bg-tertiary)', textAlign: 'center' }}>
        <h2 style={{ fontSize: 'clamp(24px, 4vw, 32px)', fontWeight: 700, marginBottom: '12px' }}>
          {t.newsletterTitle} <span className="text-gradient">{t.newsletterAccent}</span>
        </h2>
        <p className="text-secondary" style={{ maxWidth: '500px', margin: '0 auto 32px' }}>
          {t.newsletterDesc}
        </p>
        {subSuccess ? (
          <div style={{ color: 'var(--accent-primary)', fontSize: '18px', fontWeight: 600 }}>{t.newsletterSuccess}</div>
        ) : (
