...
export default function Header() {
  ...
  return (
    <>
      ...
      {/* Mobile overlay — OUTSIDE header to avoid backdrop-filter stacking context bug */}
      <div className={`nav-mobile-overlay ${menuOpen ? 'open' : ''}`}>
        ...
        <a href="https://ecommerce.infinityondemand.com.br/" target="_blank" rel="noopener noreferrer" onClick={closeMenu}>E-commerce</a>
        <a href="https://crmm.infinityondemand.com.br/" target="_blank" rel="noopener noreferrer" onClick={closeMenu}>CRM</a>
        <Link href="/delivery" onClick={closeMenu}>Delivery SaaS</Link>
        <Link href="#contact" onClick={closeMenu} className="btn-primary" style={{ padding: "12px 32px", color: "#fff" }}>Contato</Link>
      </div>
    </>
  );
}
